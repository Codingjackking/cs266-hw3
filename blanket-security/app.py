...
def predict_numbers(lottery_type):
    ...
    duration = (time.time() - start) * 1000
    log_request(f'/api/predict/{lottery_type}', request.user_id, 200, duration)
...
def run_monte_carlo_simulation(lottery_type, num_tickets):
    """Monte Carlo simulation using real historical data with security overhead"""
    # Add security overhead for blanket approach
    time.sleep(0.02)  # Simulate additional security checks
